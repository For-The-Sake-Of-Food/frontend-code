...
const MyPage = () => {
  ...
  console.log(params);
  console.log("drinks data here",drinksData);
  console.log("current index", currentIndex);
// Function to handle moving to the next index

const handleNext = () => {
  const nextIndex = (currentIndex + 1) % drinksData.length;
  const nextId = drinksData[nextIndex];
...
const handlePrevious = () => {
  const prevIndex =
    currentIndex === 0 ? drinksData.length - 1 : currentIndex - 1;
  const prevId = drinksData[prevIndex];
  navigateToRecipe(prevId,prevIndex);
};
 // Function to navigate to a specific recipe based on its index in the drinksData array
